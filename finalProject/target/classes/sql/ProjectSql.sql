...
alter table reviewBoard add constraint fk_review_id foreign key (id) references Member (id);
...
constraint fk_seller foreign key(id) references Member(id)
);
...
constraint fk_fp_seller foreign key(fp_seller) references Member(id)
);
...
select o.o_number, o.p_id, odp.qty, o.o_date, o.status, o.id, o.name, o.phone, o.totalCost, p.name "P_NAME", odp.price, p.script, p.tnImage
   							from orders o, products p,orderedproduct odp where o.p_id = p.p_id and o.o_number = odp.o_number
  			 		...
  			 		(
  		  			select o_number, p_id, qty, o_date, status, id, name, phone, totalCost, p_name, price, script, tnImage from
  		  				(
  		  	 			...
  		  	 			where id='Lee' and o_number like 'TN%'
  		  				) 
 ...
 select o_number, p_id, qty, id, name, phone, postcode, address, extraAddress, detailAddress, totalcost, status, p_name, price, script,tnImage from  
  		  	 		(
  		  	 		select o.o_number, odp.p_id, odp.qty, o.id, o.name, phone, postcode, address, extraAddress, detailAddress, totalcost, status, p.name "P_NAME", p.price, script,p.tnImage from 
  		  	 		orders o, products p, orderedproduct odp where odp.p_id = p.p_id and o.o_number = odp.o_number
  		  	 		)	
  		  	 	orders where o_number='TN2009299917' and p_id='P1237'
  		  	 	
select count(*) from orderedProduct op, orders o where op.o_number = o.o_number  and id = 'hyuk'
